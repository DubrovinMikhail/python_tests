...
def show_all_docs_info():
    all_docs_info = []
    for current_document in documents:
        all_docs_info.append(show_document_info(current_document))
...
def secretary_program_start():
    ...
    add – (add) – команда, которая добавит новый документ в каталог и в перечень полок, спросив его номер, тип,
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            for item in show_all_docs_info():
                print(item)
            print()
        elif user_command == 's':
            user_doc_number = input('Введите номер документа - ')
